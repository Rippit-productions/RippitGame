...
public class Skater : MonoBehaviour
{
    ...
    public Vector2 _FeetPosition
    {
        get
        {
            return this.transform.position - (UpVector.normalized * _CirlceCollider.radius);
        }
    }
            ...
            case SkaterState.Grind:
                {
                    ...
                    this.UpVector = _GrindAction.grindRailPoint.GetUpVector();
    ...
    protected Vector3 GetForwardMoveVector() => Vector3.Cross(UpVector, Vector3.forward);
    ...
    public void Jump()
    {
        ...
        Vector2 jumpVector = Vector2.zero + (Vector2)UpVector;
    ...
    protected (bool Success, RaycastHit2D Result) RaycastFloor()
    {
        ...
        rayHits = Physics2D.RaycastAll( _RigidBody.position, -UpVector,GetBounds().extents.x + _FloorRayLength);
        Func<RaycastHit2D, bool> valdHit = (RaycastHit2D input) =>
        {
            ...
            bool BelowBody = Vector3.Dot(UpVector, _RigidBody.position - input.point) > 0.5f;
    ...
    protected void StickToFloor()
    {
        ...
        if (Vector3.Angle(UpVector, normal) < 45.0f)
        {
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(
            transform.position,
            transform.position + (UpVector * GetBounds().size.magnitude)
         );

        // Floor Ray
        Gizmos.color = Color.red;
        Vector3 floorRayVec = -UpVector * (GetBounds().size.y / 2 + _FloorRayLength);
        Gizmos.DrawLine(
            _RigidBody.position,
