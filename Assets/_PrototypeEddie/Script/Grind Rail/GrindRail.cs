...
public class GrindRail : MonoBehaviour
{
    ...
    public Spline[] GetSplines()
    {
        ...
        if (_SplineComponent)
        {
            splines.AddRange(_SplineComponent.Splines);
        }
    ...
    private static Vector3 GetSpline2DUpVector(Spline spline, float normalPosition)
    {
        Vector3 tangent = ((Vector3)spline.EvaluateTangent(
            normalPosition)).normalized;
    ...
    {
        // Draw Mid Points on slime
        foreach (var spline in _SplineComponent.Splines)
        {
            foreach (var splineKnot in spline)
            {
                Vector3 knotPosition = (Vector3)splineKnot.Position + transform.position;
                Vector3 DownVector = (Quaternion)splineKnot.Rotation * Vector3.left;
                ...
                Vector3 fVector = GrindRail.GetSplineForwardVector(spline, SplineUtility.ConvertIndexUnit(
                    spline,
                    spline.IndexOf(splineKnot),
                    PathIndexUnit.Knot,
                    PathIndexUnit.Normalized));

                Gizmos.DrawLine(knotPosition, knotPosition + fVector);
